...
cement = pd.read_excel(r"C:\Users\ch\OneDrive\Desktop\Cement.xlsx")
...
for i in range(71):
    p = cement["Date"][i]
    cement['dates'][i]= p[0:3]
    
month_dummies = pd.DataFrame(pd.get_dummies(cement['dates']))
...
predict_data = pd.read_excel(r"C:\Users\ch\OneDrive\Desktop\Cement.xlsx")

pred_new = pd.Series(model_full.predict())
pred_new
...
new_data = pd.read_excel(r"C:\Users\psaik\OneDrive\Desktop\Cement.xlsx")
...
model1 = model.fit(Train, date_col='Date', value_col='Sales', id_col=None)
start_index = len(Train)
end_index = start_index + 11